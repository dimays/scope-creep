...
updated: 2026-09-07
---
Implement [[adr-024]]: point `DATABASE_URL` at a shared remote libSQL instance so the
console and the [[request-triage]] routine read/write the **same** threads. This is the
unlock for asynchronous write-back — without it, "submit and forget" can't post back
without the Owner launching Claude. One database, one user — **not** tenancy or auth
([[invariants]] §II preserved).

Build:
- Console runs against a remote libSQL endpoint via the existing `@libsql` client; a scoped
  token lives in env, not in the app.
