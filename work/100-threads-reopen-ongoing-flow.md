...
ongoing thread** works well, but couldn't test it because the launch→link path ([[work-099]]) is
broken.
...
3. **Reopen** the thread later → the projected transcript is still there, and the resume control
   reopens the **same** conversation (per [[work-099]]).
4. Continue the conversation in Claude → new turns appear in the projected transcript on the
   next poll.

## Dependencies
Blocked on [[work-098]] (folder) + [[work-099]] (resume-existing). This ticket is the end-to-end
...
precise Owner acceptance check rather than claimed proven. See [[work-097]], [[adr-016]].
