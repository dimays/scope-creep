...
# v2 (ADR-028): + Phase A block on agents applying the owner-approved label.
# STAGED for the Owner to copy over .claude/hooks/guard-gates.sh (agents cannot write it).
...
# --- ADR-028 Phase A / INVARIANTS v2.0.0 §4b: agents never apply the Owner-approval marker ---
# Until org authorship moves to the bot identity (Phase B, work-126), the `owner-approved`
# label is Owner evidence ONLY when the Owner applies it by hand. Block the agent-side ways
# of applying it. (String-level = defence in depth; the rule itself is INVARIANTS §4b.)
case "$cmd" in
  *"--add-label"*"owner-approved"*|*"owner-approved"*"--add-label"*)
    block "applying the 'owner-approved' label: only the Owner applies it (INVARIANTS §4b, ADR-028)";;
  *"/labels"*"owner-approved"*|*"owner-approved"*"/labels"*)
    block "applying the 'owner-approved' label via the API: only the Owner applies it (INVARIANTS §4b, ADR-028)";;
esac
