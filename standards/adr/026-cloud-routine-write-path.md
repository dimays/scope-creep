...
2. **The Owner grants the shared Claude GitHub App `Contents: write` + `Pull requests: write`, scoped to `dimays/scope-creep` + `dimays/scope-creep-console` only.** This is the identity the proxy forces, so it is the only identity that *can* be the cloud author. Explicitly **not** `Issues: write` (labels), **not** Administration / Workflows / Actions / Environments (see Consequences — blast radius).

3. **Review + merge stay entirely off the sandbox — local/human as `@scope-creep-review`.** There, identities resolve normally (no proxy), the sole code owner `@scope-creep-review` approves, and `git-manager` merges under the LIVE branch protection (owner-apply Part 0). **The routine's job ends at an open PR.**

4. **The merge boundary is enforced by *identity*, not token scoping.** Merging `main` on either repo requires an approving **code-owner** review from `@scope-creep-review` + last-push-approval + green checks. **The proxy can never present `@scope-creep-review`** — so a cloud session cannot merge *no matter what permissions the Claude App happens to hold.* This is the load-bearing safety property and it is robust to §2's exact scope.
...
- **Blast radius — grant it eyes open.** The Claude App is the identity behind **every interactive claude.ai session**, not just work-sweep. Granting it write means **any interactive session (or a prompt-injected one) can push a branch or open a PR** on the two repos. It **cannot**: merge (not the code owner), approve as code owner (`@scope-creep-review` only), or edit branch protection / workflows (no Administration/Workflows). Worst case is **spurious, non-merging, reviewable PRs — noise, not a gate breach.** For a single-user, fully-trusted-Owner system this is an acceptable trade; the containment is the code-owner gate, not the grant's tightness.

- **Caveat — the Claude App's permission set is Anthropic's to define, not ours to trim (§1d's real point, preserved).** Unlike our own `scope-creep-routine` App, the Owner **cannot hand-pick** "Contents + Pull-requests only" for the Claude App — the app requests a fixed permission manifest and the Owner consents to it for the selected repos. **So §2's scope is a request, not a guarantee.** The Owner **must record the actual granted permissions** (un-pausing criterion #3). **If the Claude App turns out to hold `Issues: write`,** an interactive/routine session could **pre-apply the `owner-approved` label** — degrading that marker's integrity (an [[adr-023]] Phase-2 concern) — **but it still cannot merge**, because the code-owner review it cannot produce is an independent required gate. The label-forge risk touches *signal trust*, never *merge capability*.
...
| 3 | **Actual permission surface recorded** | Owner reads the Claude App's real granted permissions on both repos (installation settings) | the permission list; **explicitly whether `Issues: write` is present** (label-forge caveat) |
